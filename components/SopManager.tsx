...
export const SopManager: React.FC<SopManagerProps> = ({ sopDocs, onAddSop, onRemoveSop, isAdmin }) => {
    ...
    <div className="w-full md:w-1/3 lg:w-1/4 h-1/3 md:h-full bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col">
      <div className="p-4 border-b border-gray-200 dark:border-gray-700">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400">
          {isAdmin ? 'Manage your knowledge base.' : 'Available documents.'}
        </p>
      </div>
        ...
        <ul className="space-y-2">
          ...
          {sopDocs.length === 0 && (
            <div className="text-center text-sm text-gray-400 dark:text-gray-500 py-8">
              {isAdmin ? 'No documents uploaded.' : 'No documents available.'}
            </div>
          )}
        </ul>
      </div>
